...
float PositiveCycle; // Output: Positive cycle
// internal variables
float Vacc_avg ;
float Vacc_rms ;
...
float Vacc_avg, VrmsReal=0 ;
float Vacc_rms ;
float curr_sample_norm; // normalized value of current sample
int16_t prev_sign =1;
int16_t curr_sign =1;
int16_t nsamples =0;	// samples in half cycle mains 
int16_t inv_nsamples;
int16_t inv_sqrt_nsamples;
...
void SineAnalyzer_CLA_C(SineAnalyzer v)	{													
		...
		if((v.prev_sign != v.curr_sign) && (v.curr_sign == 1)) {					
			...
			v.inv_sqrt_nsamples = sqrt(v.nsamples);							
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim->Instance == TIM1)
	{
		...
		sine_mainsV.Vin = adc_value[0]*100.01/4096.0;//voltage
		SineAnalyzer_CLA_C(sine_mainsV);	
	}
}
/* USER CODE END 0 */
/**
...
sine_mainsV.Vin=0;
sine_mainsV.SampleFreq=20000.0; // Sampling rate 20KHz
sine_mainsV.Threshold=1.65; // Threshold set to 0.02
  for(;;)
  {
